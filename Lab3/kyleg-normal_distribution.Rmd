...
1.  Make a histogram of men's heights and a histogram of women's heights.  How 
    ...
    **They could both be normal distributions. The males heights look more normal than the female heights.**
```{r 1}
hist(mdims$hgt)
hist(fdims$hgt)
```
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
    ...
    data?
    **No, they do not. They look very similar, and it confirms that the real data is normal**
```{r}
qqnorm(sim_norm)
qqline(sim_norm)
```
...
**Female weights are less normal than female heights. The distribution has a positive skew. There is more deviation in the QQ plot.**
...
6.  Write out two probability questions that you would like to answer; one 
    ...
    **The height variable has much closer agreement**

####What is the probability that a randomly chosen young adult female is less than 150 cm tall?
```{r}
pnorm(150, mean = fhgtmean, sd = fhgtsd)
sum(fdims$hgt < 150) / length(fdims$hgt)
```

####What is the probability that a randomly chosen young adult female weighs less than 50 kg?
```{r}
pnorm(50, mean = fwgtmean, sd = fwgtsd)
...
```{r}
my.dims <- data.frame(fdims$bii.di, fdims$elb.di, fdims$age, fdims$che.de)
for (i in 1:ncol(my.dims)){
  qqnorm(my.dims[,i])
  qqline(my.dims[,i])
}
```
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    **These variables are probably not as continous as the others.**
