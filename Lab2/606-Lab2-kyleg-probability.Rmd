...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    **The mode of the distribution is at 0 and it is right-skewed. The longest streak was 4 baskets**
...
kobe_streak <- calc_streak(kobe$basket)
indep_shooter <- calc_streak(sim_basket)

#par(mfrow=c(2,2)) 
barplot(table(kobe_streak), ylim = c(0,45), main = "Kobe")
barplot(table(indep_shooter), ylim = c(0,45), main = "Simulated Shooter")
summary(kobe_streak)
summary(indep_shooter)
```
Upon comparing the 2 box plots, Kobe doesn't seem to have more and longer streaks than our simulated shooter. The distributions are pretty similar.

The typical streak is a length of 0. This isn't much different than Kobe's.

The simulated player's longest streak is 6.
...
I would expect it to be somewhat similar w/ some variations. 133 shots isn't a large enough set for the Law of Large Numbers to make the p-hat converge closely on p.
...
The distributions are similar enough that I am failing to reject the null hypothesis that there are no "hot hands."
