---
title: "Kyle Gilde: Introduction to R and RStudio"
output:
  html_document:
    css: ./lab.css
    highlight: pygments
...
**There is a trend. The number of birthdays dives during the English Civil Ware period and doesn't recover until a few years after the war ended. Thereafter, it trends upward.**
...
the future.
**There appears to be the slightest trend downward over the years.**
```{r prop boys}
arbuthnot$prop.boys <- arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls)
plot(arbuthnot$year, arbuthnot$prop.boy, type = "l")
```
...
-   How do these counts compare to Arbuthnot's? Are they on a similar scale?
    **The US births are much greater in scale.**
```{r scale}
tail(arbuthnot, 10)
tail(my.present, 10)
mean(arbuthnot$boys + arbuthnot$girls)
mean(my.present$boys + my.present$girls)
```
...
-   Make a plot that displays the boy-to-girl ratio for every year in the data 
    ...
    response.
    Arbuthnot's observation of more boy's
```{r }
my.present$prop.boys <- my.present$boys / (my.present$boys + my.present$girls )
plot(x = my.present$year, y = my.present$prop.boys)
...
```{r}
subset(my.present$year, my.present$boys + my.present$girls == max(my.present$boys + my.present$girls))
plot(my.present$year, my.present$boys + my.present$girls)
```
