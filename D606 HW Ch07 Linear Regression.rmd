...
Scatterplot: Annual Murders per million as a function of % in Poverty
========================================================
...
$y = {b}_{1}x + {b}_{0}$
where ${b}_{1}$ is the slope of the line
and ${b}_{0}$ is the line's y-intercept.

![](https://raw.githubusercontent.com/kylegilde/D606-Stats/master/lmoutput.PNG)

From the regression output we know that
${b}_{1} = 2.559$ and is the slope of the line. 
${b}_{0} = -29.901$ and is the line's y-intercept.

Consequently the linear model the murder rate as a function of the poverty rate is expected as
$\^{murder} = -29.901 + 2.559{poverty\%}$

Conditions for Least Squares Line
========================================================
Before continuing, let's check the required least squares conditions to the best of our ability given that we only have the scatter plot and not the raw data.
...
3. Homoscedasticity: Having only the scatter plot, the points appear to have constant variability.
4. Independent observations: We know that the data is not from a time series, and we are probably safe to assume that the 20 metropolitan areas are independent of each other.
