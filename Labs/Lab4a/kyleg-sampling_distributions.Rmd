...
**It's right-skewed. The mean is greater than the median because of the right-skew**
...
2.  Describe the distribution of this sample. How does it compare to the 
    ...
    **The sample histogram doesn't look very much like the population. The means are nearly the same, but the sample median is actually greater than sample mean because the sample contains a disproportionate number of low values.**
...
3.  Take a second sample, also of size 50, and call it `samp2`.  How does the 
    ...
    **It's farther away from the population mean than the 1st sample statistic. The mean for the sample of 1000 would be the most accurate**
...
4.  How many elements are there in `sample_means50`?  Describe the sampling 
    ...
    **There are 5000 vectors**
    **We have a very normal-looking histogram centered on 1500.**
    **I would expect that it would look even normal**
...
5.  To make sure you understand what you've done in this loop, try running a 
    ...
    element represent?
    **There are 100 elements in the vector.**
    **Each element represents the mean of each of the 100 samples.**
```{r}
...
**The center becomes closer to the population mean, and the spread has less variance.**
...
hist(sample_means50)
print("The sampling distribution looks nearly normal, and the sampling mean appears to be approximately 180,000")
mean(price)
...
print("This sampling distribution looks nearly normal, and it appears to have less variance than the previous sampling distribution. The sampling mean appears to be approximately 180,000")
...
**#3 has the smaller spread. We would prefer distributions w/ smaller spread.**
<div id="license">
