---
title: "Kyle Gilde - Lab 1 - Introduction to data"
output:
  html_document:
    theme: cerulean
    highlight: pygments
...
```{r}
print("Cases & Variables")
dim(cdc)
#str(cdc)
var_type <- c(rep("categorical",4),rep("discrete",4),"categorical")
variable <- names(cdc)
cbind(variable, var_type)
```
...
height_summary
print("Height IQR"); 
as.numeric(height_summary[5] - height_summary[2])
print("Age")
age_summary <- summary(cdc$age)
age_summary
print("Age IQR"); 
as.numeric(age_summary[5] - age_summary[2])
print("Gender")
...
print("# of males")
sum(cdc$gender=="m")
print("Proportion reporting excellent health")
...
**Males are more likely than females to have reported that they have smoked 100 cigarettes**
...
**The general heath/BMI boxplots show that the people who reported excellent health also had lower BMIs and vice versa.**
    
**I looked at whether the BMI distibutions were any different between those who reported doing execerise in the last 30 days and those who didn't. The boxplots show that those who exercised had a lower median, 25th & 75th percentile BMIs. However, this group had a greater range overall**
...
```
    **There appears to be a positive correlation w/ desired weight being less than actual weight.**
...
If it's 0, then the person's is currently at his or her desired weight.

If negative, then the person desires to weight this many pounds less. If positive, then the person desires to weigh this many pounds more.
...
The median & mean indicate that the desired weight is less than current weight by -10 & -14.59 pounds, respectively.

The distribution is right skewed, meaning more people's desired weight was less then their current weight.

The inter-quartile range is 21 pounds.

In general, more people view their desired weight as being less than their current weight.
```{r}
summary(wdiff)
boxplot(wdiff)
...
Since the average man weighs 25% more than the average women, I looked at the weight difference in proporation to current weight. 

Yes, men and women tend to view their weight differently. The median female saw her desired weight to be 7.6% less than current weight whereas the median man's was only 2.8% less.

The mean female saw her desired weight to be 10% less than current weight whereas the mean man's was only 4.4% less.

However, by the 75th percentile, both men & women are content with their current weight.
```{r}
my.cdc <- cdc
my.cdc$wdiff <- wdiff
my.cdc$prop.wdiff <- my.cdc$wdiff/my.cdc$weight 
f.cdc <- subset(my.cdc, my.cdc$gender == "f")
m.cdc <- subset(my.cdc, my.cdc$gender == "m")

(mean(m.cdc$weight) - mean(f.cdc$weight))/mean(f.cdc$weight)
print("(% difference the average man weighs more than the average woman)")
      
boxplot(my.cdc$prop.wdiff ~ my.cdc$gender)

sum.stats <- data.frame(cbind(t(t(summary(f.cdc$prop.wdiff))), t(t(summary(m.cdc$prop.wdiff)))))
names(sum.stats) <- c("Female","Male")
sum.stats
```
...
my.cdc <- cdc
my.cdc$zscore <- zscore(my.cdc$weight)
within.one <- length(subset(my.cdc$zscore, my.cdc$zscore <= 1 & my.cdc$zscore >= -1))
within.one/nrow(my.cdc)
```
<div id="license">
