...
"typical" to mean.
**My sample appears to have a positive skew. The typical values of this sample are the mean and median: 1599 and 1546, respectively.**
```{r}
summary(samp)
hist(samp)
```
...
**It would not be identical, but it would likely be similar. They would be similar because they are both drawn from the same population, but they would not be identical due to random sampling.**
...
If you are working in this lab in a 
...
47/50
print("No, it's not exactly equal to the confidence level. Setting aside the obvious that no whole number divided by 50 will yeild .95, the .95 proportion is an expected value, and the law of large numbers indicates that as the sample size increases, the proportion will approach the expected value.")
```
...
plot_ci(lower_vector, upper_vector, mean(population))
43/50
print("It's close to the confidence level, but it is a few percentage points off.")
